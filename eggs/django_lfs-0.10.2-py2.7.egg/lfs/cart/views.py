...
def cartnovo(request, template_name="lfs/cart/cartnovo.html"):
    """
    The main view of the cart.
    """
    return render_to_response(template_name, RequestContext(request, {
...
def add_to_cart(request, product_id=None):
    ...
    # Check stock amount
    message = _(u"%(quantidade)s %(product)s ADICIONADO") % {"product": product.name,"quantidade":quantity}
    cor = "verde"
...
def refresh_cart(request):
    ...
    for item in cart.get_items():
        ...
        if item.product.manage_stock_amount and amount > item.product.stock_amount and not item.product.order_time:
            ...
            if amount == 0:
                message = _(u"Sorry, but '%(product)s' is not available anymore.") % {"product": 'PRODUTO', "amount": amount}
                cor = "vermelho"
            elif amount == 1:
                message = _(u"Sorry, but '%(product)s' is only one time available.") % {"product": 'PRODUTO', "amount": amount}
                cor = "amarelo"
            else:
                message = _(u"Sorry, but '%(product)s' is only %(amount)s times available.") % {"product": 'PRODUTO', "amount": amount}
                cor = "amarelo"
