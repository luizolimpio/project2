...
from lfs.customer.models import CreditCard,DinheiroTroco
...
class CreditCardForm(forms.ModelForm):
    ...
    class Meta:
        model = CreditCard
        exclude = ("customer","owner","number","expiration_date_month","expiration_date_year" )
    ...
    '''def clean(self):
        ...
        return self.cleaned_data'''
class DinheiroTrocoForm(forms.ModelForm):
    """
    The default credit card form which is displayed within the checkout form
    if a shop customer selects a payment method of type ``credit card``.
    """
    ...
    def __init__(self, *args, **kwargs):
        super(DinheiroTrocoForm, self).__init__(*args, **kwargs)
    ...
    def clean(self):
        ...
        payment_method_id = self.data.get("payment_method")
        payment_method = PaymentMethod.objects.get(pk=payment_method_id)
        if payment_method.type == lfs.payment.settings.PM_DINHEIRO_TROCO:
            if self.cleaned_data.get("Troco","") == '':
                self._errors["Troco"] = ErrorList([msg])
