...
from lfs.marketing.models import FeaturedProduct,Banner, Replacebusca
from lfs.marketing.form import BannerForm, ReplacebuscaForm
from django.shortcuts import render,render_to_response,get_object_or_404,resolve_url,HttpResponseRedirect
...
@permission_required("manage_shop")
def banner_view(request, template_name="manage/marketing/marketing_banner.html"):
    ...
    if request.POST:
        ...
        if form.is_valid():
            Banner.objects.create(link=form.cleaned_data['link'],imagem=form.cleaned_data['imagem'],ativo=form.cleaned_data['ativo'])
    return render_to_response(template_name, RequestContext(request, {
        "form": form,
        "banners":banners
    }))
def banner_view_update(request, pk,template_name="manage/marketing/marketing_banner.html"):
    instance = get_object_or_404(Banner, id=pk)
    form = BannerForm(request.POST or None, instance=instance)
    if form.is_valid():
        form.save()
        return HttpResponseRedirect(resolve_url("lfs_banner_view"))
...
def banner_view_delete(request, pk,template_name="manage/marketing/marketing_banner.html"):
    instance = get_object_or_404(Banner, id=pk)
    instance.delete()
    return HttpResponseRedirect(resolve_url("lfs_banner_view"))


def replacebusca(request, template_name="manage/marketing/marketing_replacebusca.html"):

    replace = Replacebusca.objects.all()
    form = ReplacebuscaForm()

    if request.POST:
        form = ReplacebuscaForm(request.POST)
        if form.is_valid():
            form.save()
    return render_to_response(template_name, RequestContext(request, {
        "form": form,
...
def replacebusca_view_update(request, pk,template_name="manage/marketing/marketing_replacebusca.html"):
    instance = get_object_or_404(Replacebusca, id=pk)
    form = ReplacebuscaForm(request.POST or None,instance=instance)
    if form.is_valid():
        form.save()
        return HttpResponseRedirect(resolve_url("lfs_replacebusca_view"))

    return render_to_response(template_name, RequestContext(request, {
        "form": form,
    }))
def replacebusca_view_delete(request, pk):
    instance = get_object_or_404(Replacebusca, id=pk)
    instance.delete()
    return HttpResponseRedirect(resolve_url("lfs_replacebusca_view"))
