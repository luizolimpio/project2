...
class Category(models.Model):
    ...
    **Attributes:**
    ...
    def get_products_personalizado(self):
        """
        Returns the direct products of the category.
        """
        cache_key = "%s-category-products-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, self.id)
        ...
        products = self.products.filter(Q(active=True , stock_amount__gte=1) | Q(active=True , manage_stock_amount=0)).exclude(sub_type=VARIANT)
        cache.set(cache_key, products)
    ...
    def get_all_products_personalizado(self):
        """
        Returns the direct products and all products of the sub categories
        """
        cache_key = "%s-category-all-products-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, self.id)
        ...
        products = lfs.catalog.models.Product.objects.distinct().filter(Q(active=True ,categories__in=categories, stock_amount__gte=1) | Q(active=True , manage_stock_amount=0, categories__in=categories)).exclude(sub_type=VARIANT).distinct()

        cache.set(cache_key, products)
        return products

    def get_all_products_personalizado_para_filtrar_categorias(self):
        """
        Returns the direct products and all products of the sub categories
        """
        cache_key = "%s-category-all-products-%s" % (settings.CACHE_MIDDLEWARE_KEY_PREFIX, self.id)
        ...
        products = lfs.catalog.models.Product.objects.distinct().filter(
                Q(active=True, categories__in=categories, stock_amount__gte=1) | Q(active=True, manage_stock_amount=0,
                                                                                   categories__in=categories)).exclude(
                sub_type=VARIANT).first()
        if products:

            Category.objects.filter(pk=self.pk).update(exclude_from_navigation=False)
        else:
            Category.objects.filter(pk=self.pk).update(exclude_from_navigation=True)
...
class Product(models.Model):
    ...
    **Parameters:**
    ...
    """
    # All products
    name = models.CharField(_(u"Name"), help_text=_(u"The name of the product."), max_length=255, blank=True)
...
class Image(models.Model):
    ...
    **Attributes:**
    ...
    title = models.CharField(_(u"Title"), blank=True, max_length=200)
    image = ImageWithThumbsField(_(u"Image"), upload_to="images", blank=True, null=True, sizes=THUMBNAIL_SIZES)
