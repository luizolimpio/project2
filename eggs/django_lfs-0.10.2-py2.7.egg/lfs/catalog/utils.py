...
def get_filtered_products_for_category(category, filters, price_filter, sorting, manufacturers_filter=None):
    ...
    if filters:
        if category.show_all_products:
            products = category.get_all_products_personalizado()
        else:
            products = category.get_products_personalizado()
        ...
        products = lfs.catalog.models.Product.objects.filter(
            Q(active=True, stock_amount__gte=1,categories__in=categories,
            sub_type__in=[STANDARD_PRODUCT, PRODUCT_WITH_VARIANTS, CONFIGURABLE_PRODUCT]) | Q(active=True, manage_stock_amount=0),categories__in=categories,
